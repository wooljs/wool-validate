...
class ValidId extends ParamCheck {
  ...
  async _check(store, param) {
    ...
    if (! (await store.has(this.as(id)))) throw new InvalidRuleError('param.check.should.exists.in.store', this, id)
    return true
  }
  asNew() {
...
class NotExistsId extends ValidId {
  ...
  async _check(store, param, t) {
    // done by the absent flag
    //if (this.containParam(param)) throw new InvalidRuleError('should not contain asNew param '+this.k)
    let id = await this.algo(t)
    this.saveToParam(param, id)
    if (await store.has(this.as(id))) {
      throw new InvalidRuleError('param.check.should.not.be.in.store',this,id)
    }
    return true
  }
}
