...
module.exports = class DictCheck extends ParamCheck {
  ...
  async _check(store, param, t) {
    ...
    if (typeof val === 'object' && !(val instanceof Array)) {
      let k = this.kvd.k
      await Promise.all(Object.entries(val).map(v => this.kvd._check(store, { [k]: v }, t)))
    } else {
      throw new InvalidRuleError('param.check.dict.invalid', JSON.stringify(val))
    }
  }
}
