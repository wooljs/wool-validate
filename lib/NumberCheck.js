...
class NumberCheck extends ParamCheck {
  ...
  asDate() {
    return new TimestampCheck(this.k, this)
  }
}
...
class PredicateNumberCheck extends NumberCheck {
  ...
  async _check(store, param) {
    if('undefined' === typeof await this.d.validate(store, param)) {
      ...
      if (! this.f(v)) throw this.buildError('param.invalid.num.predicate', v, this.printPredicate())
    }
  }
  printPredicate() {
...
class TimestampCheck extends IntegerCheck {
  constructor(k, d) {
    super(k, d)
  }
  async _check(store, param) {
