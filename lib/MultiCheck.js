...
module.exports = class MultiCheck extends ParamCheck {
  ...
  absent(force_check = false) {
    const r = this.clone(this.p.map(x => x.absent(force_check)))
    r.setAbsent(force_check)
    return r
  }
  present() {
    const r = this.clone(this.p.map(x => x.present()))
    r.presence = ParamCheck.Presence.present
    return r
  }
  optional() {
    const r = this.clone(this.p.map(x => x.optional()))
    r.presence = ParamCheck.Presence.optional
    return r
  }
  default(obj) {
    const r = this.clone(this.p.map(x => x.k in obj ? x.default(obj[x.k]) : x ))
    r.presence = ParamCheck.Presence.default
    r.defaultValue = obj
    return r
  }
  static build(p = []) {
